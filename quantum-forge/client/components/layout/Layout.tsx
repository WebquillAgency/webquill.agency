    ...
    <header className="sticky top-2 md:top-4 z-40">
      <div className="mx-auto max-w-5xl px-2">
        ...
        <div className={`fixed inset-0 z-50 md:hidden ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
          ...
          />
          <div
            className={`absolute left-0 top-0 h-full w-72 max-w-[80%] border-r border-white/10 bg-black/80 p-4 transition-transform duration-300 ease-out ${open ? "translate-x-0" : "-translate-x-full"}`}
          >
            <div className="mb-3 flex items-center justify-between">
              <span className="text-sm font-semibold text-white">Menu</span>
              <button aria-label="Close menu" className="rounded-md p-1 text-slate-200 hover:bg-white/10" onClick={() => setOpen(false)}>
                <svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor"><path d="M18.3 5.7 12 12l6.3 6.3-1.4 1.4L10.6 13.4 4.3 19.7 2.9 18.3 9.2 12 2.9 5.7 4.3 4.3 10.6 10.6 16.9 4.3z"/></svg>
              </button>
            </div>
            <div className="flex flex-col gap-2">
              {links.map((l) => (
                <a key={l.href} href={l.href} className="rounded-md px-2 py-2 text-slate-200 hover:bg-white/10" onClick={() => setOpen(false)}>
                  {l.label}
                </a>
              ))}
              <a href="#contact" className="mt-2 inline-flex items-center justify-center rounded-md bg-gradient-to-br from-amber-400 to-yellow-300 px-4 py-2 text-sm font-semibold text-black hover:from-amber-300 hover:to-yellow-200" onClick={() => setOpen(false)}>
                Get started
              </a>
            </div>
          </div>
